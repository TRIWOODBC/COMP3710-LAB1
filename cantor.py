...
def cantor_mask(level: int, width: int, device: str ) -> torch.Tensor:
    ...
    for _ in range(level):
        ...
        t = torch.div(t, 3, rounding_mode='floor') # integer division ternary digit
    return mask

def cantor_ladder(device: str, n_levels=6, width=1200, bar_thickness=10, row_gap=8):
    ...
    for level in range(n_levels + 1):
        ...
        mask3d = mask1d.view(1, -1, 1).expand(bar_thickness, width, 3) # match the bar area
        color  = colors[level].view(1, 1, 3).expand(bar_thickness, width, 3)
        band = img[y:y+bar_thickness, :, :] #Slice out the current bar region to be filled.
